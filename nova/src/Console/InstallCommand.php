...
class InstallCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->comment('Generating UserResource Resource...');
        $this->callSilent('nova:resource', ['name' => 'UserResource']);
        copy($this->resolveStubPath('/stubs/nova/user-resource.stub'), app_path('Nova/UserResource.php'));

        if (file_exists(app_path('Models/UserResource.php'))) {
            file_put_contents(
                app_path('Nova/UserResource.php'),
                str_replace('App\User::class', 'App\Models\User::class', file_get_contents(app_path('Nova/UserResource.php')))
            );
        }
    ...
    protected function setAppNamespace()
    {
        ...
        $this->setAppNamespaceOn(app_path('Nova/UserResource.php'), $namespace);
